...
REGENERATED = (
    "JWT_SECRET", "GEO_CRON_KEY", "SEO_CRON_KEY", "MR_CRON_KEY", "INBOX_CRON_KEY",
)

#: Regenerated too, but SHAPED: a Fernet key is 32 random bytes urlsafe-base64
#: encoded, and ``token_urlsafe`` is not one. Generated by the library that will
#: read it, so the two can never disagree about the format. Staging gets its
#: own so a staging database copied from production could never open a
#: production Gmail grant.
FERNET_KEYS = ("INBOX_TOKEN_KEY",)
...
def staging_env(prod: dict) -> list[dict]:
    ...
    for key in FERNET_KEYS:
        if key in env:
            ...
            env[key] = Fernet.generate_key().decode()
