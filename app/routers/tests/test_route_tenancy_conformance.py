...
ROUTE_LEDGER: dict[tuple[str, str], tuple[str, str]] = {
    ...
    # --- the Inbox Triage agent (a12), open to every signed-in user --------- #
    # Every one of these reads and writes ``inbox_triage/{caller's user id}``
    # and nothing else — the id comes from the token, never the request — so
    # the Gmail grant, the sheet reference and the disconnect all belong to
    # the caller. Plain ``get_current_user``; a GEO-only account is refused
    # by the scope wall, which the INTERNAL_ONLY sweep below proves per route.
    ("POST", "/api/inbox/disconnect"): (TENANT_SCOPED, INTERNAL_ONLY),
    ("POST", "/api/inbox/oauth/complete"): (TENANT_SCOPED, INTERNAL_ONLY),
    ("POST", "/api/inbox/oauth/start"): (TENANT_SCOPED, INTERNAL_ONLY),
...
#:
#: 70 → 77 on 2026-09-25: self-serve Graphics Designer brands. Seven routes
