...
# The transport layer — ADC once per process, a bounded token refresh, an
# httplib2 transport with a socket deadline, and the transient-failure retry —
# moved to ``app.services.google_http`` on 2026-09-18 so the Inbox Triage
# agent's Gmail and Sheets clients share it instead of copying it. The names
# are re-exported here because this module is where the MR suite and the
# deadline sweep look for them; ``_execute`` below keeps its "Sheets …" wording
# and raises :class:`SheetsUnavailable`, exactly as before.
from app.services.google_http import (  # noqa: E402,F401 — re-exported on purpose
...
def _execute(request, *, what: str):
    ...
    The retry itself lives in :func:`app.services.google_http.execute_with_retry`;
    this binds it to the "Sheets …" wording and to :class:`SheetsUnavailable`,
    which is what every caller in this agent is written against.
    """
    return execute_with_retry(request, what=f"Sheets {what}", unavailable=SheetsUnavailable)
