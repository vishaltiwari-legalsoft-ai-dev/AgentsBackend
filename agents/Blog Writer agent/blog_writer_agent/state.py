"""Persistence gate: Firestore in cloud mode, local JSON files when BLOG_OFFLINE=1.

Doc ids use ``-`` separators only (``run-{id}``, ``inventory-{brand}``,
``runs-index``) so the local fallback maps them 1:1 to Windows-safe filenames.
Mirror of ``seo_geo_agent.state`` with its own collection so blog runs never
collide with SEO documents.
"""
...
_COLLECTION = "blog_writer"


def use_cloud() -> bool:
    return os.environ.get("BLOG_OFFLINE", "0") != "1"


def _local_dir() -> Path:
    raw = os.environ.get("BLOG_LOCAL_DIR", "")
