...
def _build_prompt(layout_desc: str) -> str:
    return (
        ...
        "legible, and no new text of any kind was added anywhere (an added label, "
        "tag, stamp, watermark or vertical micro-text is a violation; a "
        "repositioned block is fine; reworded/restyled/reflowed text is not)\n"
        "2. elements_ok — every pre-existing shape/icon/button still present and "
        "identical in look (repositioning to fix an overlap is fine; new NON-TEXT "
        "emphasis like a glow or accent stroke is acceptable)\n"
